...
cfssl gencert \
      ...
      -ca-key=output/certs/ca-key.pem \
      -config=output/certs/config/ca_config.json \
      -hostname=192.168.1.50,10.0.2.1,10.0.3.1,127.0.0.1,yard-1,kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.svc.cluster.local \
      -profile=kubernetes \
